...
class LabelSmoothCEsim(nn.Module):
    '''
    This is the autograd version, you can also try the LabelSmoothSoftmaxCEV2 that uses derived gradients
    '''

    def __init__(self, lb_smooth=0.1, reduction='mean', ignore_index=-100, word_emb_tab=None, norm_type='softmax', temp=1.0, variant='word_sim_softmax_0.2_0.5'):#None
        super(LabelSmoothCEsim, self).__init__()
    ...
    def forward(self, logits, label, feature_emeb, center_emb, topk_idx=None, mixup_lam=None, y_a=None, y_b=None, **kwargs):
        '''
        ...
        with t.no_grad():
            ...
            if self.variant is None:
                ...
                word_emb_sim = t.matmul(F.normalize(feature_emeb, dim=-1), F.normalize(center_emb, dim=-1).T)
                #word_emb_sim_t = t.matmul(F.normalize(feature_emeb, dim=-1), F.normalize(feature_emeb, dim=-1).T)
                lb_one_hot = word_emb_sim
                lb_one_hot[t.arange(lb_one_hot.size(0)), label] = 1
                ignore = label.eq(self.lb_ignore)
                n_valid = ignore.eq(0).sum()
        ...
        ignore_mask = (label == self.lb_ignore)
        loss[ignore_mask] = 0
